...
class Role extends Model
{
    ...
    */
    public function whereData($where=array(),$field="*",$orderby=''){
        return DB::table('role')->where($where)->select($field)->orderBy("id","desc")->get()->toarray();
    }
      /**
    * @return mixed  查找数据
    */
    public function arrData($where=array(),$field="*",$orderby=''){
        return  DB::table('role')->where($where)->select($field)->orderBy("list_order", "ASC")->get()->toArray();
    }
    /**
    * @return mixed  查找数据
    */
    public function datasFind($where='',$field='*'){
     // return DB::select('select * from admin where admin_id = ?', [$id]);
        if(is_array($where)){
            return DB::table('role')->select($field)->where($where)->first();
        }else{
            return false;
        }
    }
    ...
    */
    public function updatas($where,$data){
        if(is_array($where)&&is_array($data)){
            return DB::table('role')->where($where)->update($data);
        }else{
            return false;
        }
    }
    ...
    */
    public function deletes($where){
        if(is_array($where)){
            return DB::table('role')->where($where)->delete();
        }else{
            return false;
        }
    }
