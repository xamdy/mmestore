...
<body>
	<div class="wrap">
        ...
        <ul class="nav nav-tabs">
            ...
            <li class="active"><a href="<?php echo e(url('admin/container/edit')); ?>">编辑体验店</a></li>
        </ul>

		<form method="post" class="form-horizontal js-ajax-form margin-top-20" id="form" action="<?php echo e(url('admin/container/edit')); ?>">
			<input type="hidden" name="_token" value="<?php echo e(csrf_token()); ?>">
			<div class="form-group">
				<label for="input-user_login" class="col-sm-2 control-label">体验店编号</label>
				<div class="col-md-2 col-sm-10">
					<input type="text" class="form-control" readonly value="<?php echo e($res['info']->container_number); ?>" id="container_number" name="container_number">
					<input type="hidden" class="form-control"  value="<?php echo e($res['info']->id); ?>" id="id" name="id">
				</div>
			</div>
			<div class="form-group">
				<label for="input-user_login" class="col-sm-2 control-label">体验店锁编号</label>
				<div class="col-md-2 col-sm-10">
					<input type="text" class="form-control"  value="<?php echo e($res['info']->lock_code); ?>" onBlur="update_code()" id="code" name="code">
					<input type="hidden" class="form-control" readonly  value="<?php echo e($res['info']->lock_code); ?>" id="lock_code" >
				</div>
			</div>
			<div class="form-group">
				<label  class="col-sm-2 control-label">体验店状态</label>
				<input type="hidden" class="form-control" readonly  value="<?php echo e($res['info']->status); ?>" id="status_bk" >
				<div class="col-md-2 col-sm-10">
					<select class="form-control" name="status" id="status" onchange="update_status()">
						<option value="1"  <?php echo e($res['info']->status== '1' ? 'selected'  :''); ?> >使用中</option>
						<option value="2"  <?php echo e($res['info']->status== '2' ? 'selected'  :''); ?> >维修中</option>
						<option value="3"  <?php echo e($res['info']->status== '3' ? 'selected'  :''); ?> >废弃</option>
					</select>
				</div>
			</div>
					...
					<table class="table table-hover table-bordered">
						...
						<?php $__currentLoopData = $res['goods']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $vo): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
							<tr id="goods_<?php echo e($vo->barcode); ?>">
								<td> <input type='hidden' class='lzt' value="<?php echo e($vo->barcode); ?>" id="" name="goods[<?php echo e($vo->goods_id); ?>][barcode]">
									  <input type='hidden'  value="<?php echo e($vo->goods_id); ?>" name="goods[<?php echo e($vo->goods_id); ?>][goods_id]">
									<input type='hidden'  value="<?php echo e($vo->inventory_status); ?>" id="inventory_status_<?php echo e($vo->barcode); ?>" >
										<?php echo e($vo->barcode); ?>

								</td>
								<td><?php echo e($vo->goods_name); ?></td>
								<td><?php echo e($vo->present_price); ?></td>
								<td><input type='text' style='width: 30px;' value='<?php echo e($vo->level); ?>'  name="goods[<?php echo e($vo->goods_id); ?>][level]">

								</td>
								<td><a href='#' onclick='del_goods(<?php echo e($vo->barcode); ?>)' ><i class='fa fa-minus-circle normal'></i></a></td>
							</tr>
...
<script>
	...
	function add_goods(){
		...
		layer.open({
			...
			,yes: function(index, layero){
				...
				if (reg.exec(num)) {
					//提交输入信息并返回data数据
					var url = "<?php echo e(url('admin/container/checkGoods')); ?>";
					$.ajax({
						type:"post",
						url:url,
	...
	function update_do(status){
		...
		var url = "<?php echo e(url('admin/container/updateStatus')); ?>";
		$.ajax({
			type:"post",
			url:url,
